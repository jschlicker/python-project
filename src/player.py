...
class Player(pygame.sprite.Sprite):
	...
	def player_assets(self):
		...
		# building path for each element for importing it
	...
	# Checking status and updating it
	def status_update(self):
		...
		# blocking other status when attacking
		if self.attacking:
			self.direction.x = 0
			self.direction.y = 0
		...
		# removing attacking status after attacking
		else:
	...
	def animate(self):
		...
		# loop over frame index to create image sequence for animation
		self.frame_index += self.animation_speed
		if self.frame_index >= len(animation):
			self.frame_index = 0

		# setting the image depending on frame index 
